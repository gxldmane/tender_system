...
export default function Browse() {
  ...
  const { data: categoriesResponse, isFetching: isCategoriesFetching } = useQuery({
    queryKey: ['categories'],
    queryFn: () => httpClient.getCategories(),
    select: data => data?.data as ICategoriesResponse,
  });

  const { data: regionsResponse, isFetching: isRegionsFetching } = useQuery({
    queryKey: ['regions'],
          ...
          <div className="flex justify-end grow mt-4">
            <Pagination className='mt-auto'>
              {!isFetching && <PaginationContent>
                <PaginationItem>
                  <PaginationPrevious className={cn(!response.links.prev && "invisible")} href={createPageURL(currentPage - 1)} />
                </PaginationItem>
                ...
                <PaginationItem>
                  <PaginationNext className={cn(!response.links.next && "invisible")} href={createPageURL(currentPage + 1)} />
                </PaginationItem>
              </PaginationContent>}
            </Pagination>
          </div>
